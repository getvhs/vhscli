...
export function register(parent: Command, mode: Mode) {
  parent.command("seedream-5")
    .description(mode === "submit" ? "submit a seedream 5.0 image task" : "generate an image with seedream 5.0")
    ...
    .showHelpAfterError(`(run 'vhscli ${mode} seedream-5 --help' for usage)`)
    .addHelpText("after", `
...
examples:
  vhscli ${mode} seedream-5 "a girl in a yellow raincoat walking under a parasol, monet oil painting style"
  vhscli ${mode} seedream-5 "remove her hat, keep everything else" -i photo.jpg`)
...
async function run(prompt_arg: string, opts: Opts, mode: Mode) {
  ...
  const output = opts.output ?? default_output("seedream-5", "jpg")
