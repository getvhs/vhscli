...
async function run(prompt_arg: string, opts: Opts) {
  ...
  const { result } = await submit(sess, "byteplus:seedream-4-5", payload, "generating image...", 300_000)
...
async function parse_opts(sess: Session, prompt_arg: string, opts: Opts) {
  ...
  return zparse(schema.request, payload, "bad seedream-4-5 payload")
}

export async function save(result: unknown, output: string | null) {
  const item = zparse(schema.response, result, "bad seedream-4-5 response").data[0]!
